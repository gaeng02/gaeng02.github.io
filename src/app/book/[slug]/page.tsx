...
export async function generateStaticParams() {
  ...
  return posts
    .filter((post) => post.category === 'book')
    .map((post) => ({
      slug: post.slug,
    }))
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const post = getPostBySlug('book', slug)
...
export default async function BookPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const post = getPostBySlug('book', slug)
  ...
  const jsonLd = {
    ...
    publisher: {
      ...
      logo: {
        '@type': 'ImageObject',
        url: 'https://gaeng02.github.io/assets/images/logo.png',
      },
    },
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://gaeng02.github.io/book/${slug}`,
    },
